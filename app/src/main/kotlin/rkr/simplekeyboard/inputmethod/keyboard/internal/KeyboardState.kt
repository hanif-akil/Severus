...
class KeyboardState(private val mSwitchActions: SwitchActions) {
    private var mShiftKeyState = ShiftKeyState("Shift")
    private var mSymbolKeyState = ModifierKeyState("Symbol")
    private var mSwitchState = SWITCH_STATE_ALPHA
    private var mIsAlphabetMode = false
    private var mAlphabetShiftState = AlphabetShiftState()
    ...
    private fun toggleAlphabetAndSymbols(autoCapsFlags: Int, recapitalizeMode: Int) {
        ...
        if (mIsAlphabetMode) {
            ...
            if (mPrevSymbolsKeyboardWasShifted) setSymbolsShiftedKeyboard()
            else setSymbolsKeyboard()
            mPrevSymbolsKeyboardWasShifted = false
        } else {
    ...
    private fun toggleShiftInSymbols() {
        if (mIsSymbolShifted) setSymbolsKeyboard()
        else setSymbolsShiftedKeyboard()
    }
    ...
    fun onPressKey(code: Int, isSinglePointer: Boolean, autoCapsFlags: Int, recapitalizeMode: Int) {
        ...
        when (code) {
            ...
            Constants.CODE_CAPSLOCK -> { /* Nothing to do here */ }
    ...
    private fun onReleaseShift(withSliding: Boolean, autoCapsFlags: Int, recapitalizeMode: Int) {
        if (RecapitalizeStatus.NOT_A_RECAPITALIZE_MODE != mRecapitalizeMode) {
            ...
            if (mIsInDoubleTapShiftKey) {
                ...
                if (mAlphabetShiftState.isShiftLockShifted()) setShiftLocked(true)
                else setShifted(UNSHIFT)
                mShiftKeyState.onRelease()
                mSwitchActions.requestUpdatingShiftState(autoCapsFlags, recapitalizeMode)
                return
    ...
    override fun toString(): String {
        return "[keyboard=${if (mIsAlphabetMode) mAlphabetShiftState.toString()
        else if (mIsSymbolShifted) "SYMBOLS_SHIFTED" else "SYMBOLS"}" +
                " shift=$mShiftKeyState symbol=$mSymbolKeyState switch=${switchStateToString(mSwitchState)}]"
    }
