...
class AppearanceSettingsFragment : SubScreenFragment() {
    ...
    private fun setupKeyboardHeightSettings() {
        val pref = findPreference(Settings.PREF_KEYBOARD_HEIGHT) as? SeekBarDialogPreference
            ?: return
        val prefs = sharedPreferences
        val res = resources
        pref.setInterface(object : SeekBarDialogPreference.ValueProxy {
            ...
            override fun getValueText(value: Int): String {
                return if (value < 0) res.getString(R.string.settings_system_default)
                else res.getString(R.string.abbreviation_unit_percent, value)
            }
    ...
    private fun setupBottomOffsetPortraitSettings() {
        val pref = findPreference(Settings.PREF_BOTTOM_OFFSET_PORTRAIT) as? SeekBarDialogPreference
            ?: return
        val prefs = sharedPreferences
        val res = resources
        pref.setInterface(object : SeekBarDialogPreference.ValueProxy {
            ...
            override fun getValueText(value: Int): String {
                return if (value < 0) res.getString(R.string.settings_system_default)
                else res.getString(R.string.abbreviation_unit_dp, value)
            }
    ...
    private fun setupKeyboardColorSettings() {
        val pref = findPreference(Settings.PREF_KEYBOARD_COLOR) as? ColorDialogPreference
            ?: return
        val prefs = sharedPreferences
