...
object SubtypeLocaleUtils {
    ...
    private val sSupportedLocales = arrayOf(
        LOCALE_ENGLISH_UNITED_STATES, LOCALE_ENGLISH_GREAT_BRITAIN,
        LOCALE_AFRIKAANS, LOCALE_ARABIC, LOCALE_AZERBAIJANI_AZERBAIJAN,
        LOCALE_BELARUSIAN_BELARUS, LOCALE_BULGARIAN,
        LOCALE_BENGALI_BANGLADESH, LOCALE_BENGALI_INDIA,
        LOCALE_CATALAN, LOCALE_CZECH, LOCALE_DANISH,
        LOCALE_GERMAN, LOCALE_GERMAN_SWITZERLAND, LOCALE_GREEK,
        LOCALE_ENGLISH_INDIA, LOCALE_ESPERANTO,
        LOCALE_SPANISH, LOCALE_SPANISH_UNITED_STATES, LOCALE_SPANISH_LATIN_AMERICA,
        LOCALE_ESTONIAN_ESTONIA, LOCALE_BASQUE_SPAIN,
        LOCALE_PERSIAN, LOCALE_FINNISH,
        LOCALE_FRENCH, LOCALE_FRENCH_CANADA, LOCALE_FRENCH_SWITZERLAND,
        LOCALE_GALICIAN_SPAIN, LOCALE_HINDI, LOCALE_CROATIAN, LOCALE_HUNGARIAN,
        LOCALE_ARMENIAN_ARMENIA, LOCALE_INDONESIAN_1, LOCALE_INDONESIAN_2,
        LOCALE_ICELANDIC, LOCALE_ITALIAN, LOCALE_ITALIAN_SWITZERLAND,
        LOCALE_HEBREW_1, LOCALE_HEBREW_2, LOCALE_GEORGIAN_GEORGIA,
        LOCALE_KAZAKH, LOCALE_KHMER_CAMBODIA, LOCALE_KANNADA_INDIA,
        LOCALE_KYRGYZ, LOCALE_LAO_LAOS, LOCALE_LITHUANIAN, LOCALE_LATVIAN,
        LOCALE_MACEDONIAN, LOCALE_MALAYALAM_INDIA, LOCALE_MONGOLIAN_MONGOLIA,
        LOCALE_MARATHI_INDIA, LOCALE_MALAY_MALAYSIA,
        LOCALE_NORWEGIAN_BOKMAL, LOCALE_NEPALI_NEPAL,
        LOCALE_DUTCH, LOCALE_DUTCH_BELGIUM, LOCALE_POLISH,
        LOCALE_PORTUGUESE_BRAZIL, LOCALE_PORTUGUESE_PORTUGAL,
        LOCALE_ROMANIAN, LOCALE_RUSSIAN, LOCALE_SAKHA,
        LOCALE_SLOVAK, LOCALE_SLOVENIAN, LOCALE_SERBIAN, LOCALE_SERBIAN_LATIN,
        LOCALE_SWEDISH, LOCALE_SWAHILI,
        LOCALE_TAMIL_INDIA, LOCALE_TAMIL_SINGAPORE, LOCALE_TELUGU_INDIA,
        LOCALE_THAI, LOCALE_TAGALOG, LOCALE_TURKISH,
        LOCALE_UKRAINIAN, LOCALE_URDU, LOCALE_UZBEK_UZBEKISTAN,
        LOCALE_VIETNAMESE, LOCALE_ZULU
    )

    fun getSupportedLocales(): List<String> = sSupportedLocales.toList()
    ...
    fun getDefaultSubtype(locale: String, resources: Resources): Subtype? {
        val subtypes = SubtypeBuilder(locale, true, resources).getSubtypes()
        return if (subtypes.isEmpty()) null else subtypes[0]
    }

    fun getSubtype(locale: String, layoutSet: String, resources: Resources): Subtype? {
        val subtypes = SubtypeBuilder(locale, layoutSet, resources).getSubtypes()
        return if (subtypes.isEmpty()) null else subtypes[0]
    }
    ...
    fun getDefaultSubtypes(resources: Resources): List<Subtype> {
        ...
        for (systemLocale in systemLocales) {
            ...
            if (bestLocale != null && !addedLocales.contains(bestLocale)) {
                ...
                if (bestSubtype != null) subtypes.add(bestSubtype)
            }
        }
        if (subtypes.isEmpty()) {
    ...
    private class SubtypeBuilder {
        ...
        fun getSubtypes(): List<Subtype> {
            mSubtypes?.let { return it }
            val subtypes = ArrayList<Subtype>()
            mSubtypes = subtypes
            when (mLocale) {
                ...
                LOCALE_RUSSIAN, LOCALE_UKRAINIAN -> addLayout(LAYOUT_EAST_SLAVIC)
                LOCALE_BULGARIAN -> {
                ...
                LOCALE_HEBREW_1, LOCALE_HEBREW_2 -> addLayout(LAYOUT_HEBREW)
                ...
                LOCALE_TAMIL_INDIA, LOCALE_TAMIL_SINGAPORE -> addLayout(LAYOUT_TAMIL)
        ...
        private fun shouldSkipLayout(keyboardLayoutSet: String): Boolean {
            if (mAllowMultiple) return false
            if (mSubtypes!!.size > 0) return true
            if (mExpectedLayoutSet != null) return mExpectedLayoutSet != keyboardLayoutSet
            return false
        }

        private fun addLayout(keyboardLayoutSet: String) {
            if (shouldSkipLayout(keyboardLayoutSet)) return
            ...
            mSubtypes!!.add(Subtype(mLocale, keyboardLayoutSet, layoutNameStr, false, mResources))
        }

        private fun addLayout(keyboardLayoutSet: String, layoutRes: Int) {
            if (shouldSkipLayout(keyboardLayoutSet)) return
            mSubtypes!!.add(Subtype(mLocale, keyboardLayoutSet, layoutRes, true, mResources))
        }

        private fun addGenericLayouts() {
            if (mSubtypes!!.size > 0 && !mAllowMultiple) return
            ...
            for (i in predefinedKeyboardLayoutSets.indices) {
                ...
                if (shouldSkipLayout(predefinedLayout)) continue
                ...
                mSubtypes!!.add(
                    Subtype(mLocale, predefinedLayout, predefinedKeyboardLayoutSetDisplayNames[i], true, mResources)
                )
            }
        }
    }
